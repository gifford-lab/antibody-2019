...
def model(X_train, Y_train, X_test, Y_test):
    ...
    model = Sequential()
    model.add(Convolution2D(32, 1, 5, border_mode='same', input_shape=(20, 1, DATASIZE),activation='relu',W_constraint=maxnorm(W_maxnorm)))
    model.add(MaxPooling2D(pool_size=(1, 2),strides=(1,2)))
    model.add(Convolution2D(64, 1, 5, border_mode='same', activation='relu',W_constraint=maxnorm(W_maxnorm)))
    model.add(MaxPooling2D(pool_size=(1, 2),strides=(1,2)))
    model.add(Flatten())

    model.add(Dense(16,activation='relu',W_constraint=maxnorm(W_maxnorm)))
